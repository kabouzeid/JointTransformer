...
class HandMeshTSVDataset(object):
    ...
    def __getitem__(self, idx):
        ...
        inputs["img"] = transfromed_img

        targets["mano.pose.r"] = meta_data["pose"]
        targets["mano.beta.r"] = meta_data["betas"]
        targets["mano.j2d.norm.r"] = meta_data["joints_2d"][:, :2]
        targets["mano.j3d.full.r"] = meta_data["joints_3d"][:, :3]
        targets["is_valid"] = float(meta_data["has_smpl"])
        targets["right_valid"] = targets["is_valid"]
        targets["joints_valid_r"] = np.ones(21) * targets["right_valid"]

        meta_info["imgname"] = img_key
        # meta_info["intrinsics"] = torch.tensor(
        #     [[1000, 0, 112], [0, 1000, 112], [0, 0, 1]]
        # ).float()
        # assert transfromed_img.shape[1] == 224
